(() => {
  // ../edp_online_vehicles/edp_online_vehicles/public/js/attach.js
  frappe.ui.form.ControlAttach = class ControlAttach extends frappe.ui.form.ControlData {
    make_input() {
      let me = this;
      this.$input = $('<button class="btn btn-default btn-sm btn-attach">').html(__("Attach")).prependTo(me.input_area).on({
        click: function() {
          me.on_attach_click();
        },
      ...
      if (this.frm) {
        ...
        me.frm.attachments.remove_attachment_by_filename(
          ...
          async () => {
            ...
            me.refresh();
            if (cur_frm.doc.doctype != "Vehicles Order" && cur_frm.doc.doctype != "Dealer Claims") {
    ...
    toggle_reload_button() {
      this.$value.find('[data-action="reload_attachment"]').toggle(
        this.file_uploader && this.file_uploader.uploader.files.length > 0
      );
    }
  };
})();
//# sourceMappingURL=edp_online_vehicles1.bundle.6VQSCLTL.js.map
