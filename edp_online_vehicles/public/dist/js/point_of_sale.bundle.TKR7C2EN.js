(() => {
  ...
  // ../edp_online_vehicles/edp_online_vehicles/edp_online_vehicles/page/part_order_1/pos_item_selector.js
  ...
  edp_online_vehicles.PartOrder.ItemSelector = class {
    ...
    bind_events() {
      ...
      this.$component.on("click", ".item-wrapper", async function() {
        ...
        if (hq === 0 && dealer > 0) {
          try {
            let { ordered_qty, company, is_backorder } = await showDealerOrderPopup(part_no);
            if (is_backorder) {
              backorder = true;
            }
    ...
    resize_selector(minimize) {
      ...
      minimize ? this.$items_container.css(
    ...
    return new Promise((resolve, reject) => {
      frappe.call({
        ...
        callback: function(r) {
          ...
          let dealerRows = dealers.map((dealer) => {
            let maxAttribute = dealer.allow_backorder ? "" : `max="${dealer.soh}"`;
            return `
          ...
          function updateOrderButtonState() {
            ...
            $(".order-btn").prop("disabled", totalQty === 0).css(
              "cursor",
          ...
          $(".qty-plus").click(function() {
            ...
            let maxQty = $input.attr("max") ? parseInt($input.attr("max"), 10) : null;
          ...
          $(".qty-input").on("input", function() {
            let maxQty = $(this).attr("max") ? parseInt($(this).attr("max"), 10) : null;
  ...
  // ../edp_online_vehicles/edp_online_vehicles/edp_online_vehicles/page/part_order_1/pos_item_cart.js
  edp_online_vehicles.PartOrder.ItemCart = class {
    ...
    attach_shortcuts() {
      for (let row of this.number_pad.keys) {
        for (let btn of row) {
          ...
          let shortcut_label = "";
          if (shortcut_key) {
            shortcut_label = shortcut_key.split("+").map(frappe.utils.to_title_case).join("+");
    ...
    render_vat_total(value) {
      const currency = this.events.get_frm().doc.currency;
      this.$totals_section.find(".vat-total-container").html(
        `<div>${__("VAT Total")}</div><div>${format_currency(value, currency)}</div>`
      );
      this.$numpad_section.find(".numpad-vat-total").html(
    ...
    render_cart_item(item_data, $item_to_update) {
      ...
      function get_rate_discount_html() {
        if (item_data.dealer_billing_excl && item_data.total_excl && item_data.dealer_billing_excl !== item_data.total_excl) {
  ...
  // ../edp_online_vehicles/edp_online_vehicles/edp_online_vehicles/page/part_order_1/pos_item_details.js
...
`);
  ...
  // ../edp_online_vehicles/edp_online_vehicles/edp_online_vehicles/page/part_order_1/pos_number_pad.js
        ...
        return keys.reduce((a, row, i) => {
          return a + row.reduce((a2, number, j) => {
            ...
            if (number == "Discount" || number == "Rate") {
              return a2 + `<div class="numpad-btn ${class_to_append}" data-button-value="${fieldname}" style="visibility:hidden">${__(
                number
              )}</div>`;
            } else {
  ...
  // ../edp_online_vehicles/edp_online_vehicles/edp_online_vehicles/page/part_order_1/pos_payment.js
      ...
      frappe.db.get_doc("POS Settings", void 0).then((doc) => {
        ...
        fields.forEach((df) => {
          ...
          if (df.fieldtype == "Button") {
            df_events = {
              click: function() {
                if (frm.script_manager.has_handlers(
                  df.fieldname,
                  frm.doc.doctype
                )) {
  ...
  // ../edp_online_vehicles/edp_online_vehicles/edp_online_vehicles/page/part_order_1/pos_past_order_list.js
...
Return`,
  ...
  // ../edp_online_vehicles/edp_online_vehicles/edp_online_vehicles/page/part_order_1/pos_past_order_summary.js
  ...
  // ../edp_online_vehicles/edp_online_vehicles/edp_online_vehicles/page/part_order_1/pos_controller.js
...
`);
      ...
      this.payment = new edp_online_vehicles.PartOrder.Payment({
        ...
        events: {
          ...
          toggle_other_sections: (show) => {
            if (show) {
              this.item_details.$component.is(":visible") ? this.item_details.$component.css(
                "display",
                "none"
              ) : "";
    ...
    init_recent_order_list() {
      this.recent_order_list = new edp_online_vehicles.PartOrder.PastOrderList({
        wrapper: this.$components_wrapper,
        events: {
    ...
    async on_cart_update(args) {
      ...
      try {
        ...
        const item_row_exists = item_row !== void 0;
        const from_selector = field === "qty" && value === "+1";
        if (from_selector)
          value = flt((item_row == null ? void 0 : item_row.qty) || 0) + flt(value);
        if (item_row_exists) {
          ...
          if (rate == void 0 || rate == 0) {
            if (dealer_billing_excl == void 0 || dealer_billing_excl == 0) {
    ...
    get_item_from_frm({ name, part_no, batch_no, uom, rate }) {
      ...
      if (name) {
        ...
        item_row = this.frm.doc.table_avsu.find(
          (i) => i.part_no === part_no && i.dealer_billing_excl === flt(rate)
        );
      }
      return item_row || {};
    }
