...
def get_items(start, page_length, price_list, item_group, search_term=""):
	...
	condition += get_item_group_condition()
	...
	for item in items_data:
		...
		# in warehouses whose company is not the HQ company and where qty > 0.
		dealer_qty_data = frappe.db.sql(
			"""
            SELECT IFNULL(SUM(bin.actual_qty), 0) as dealer_qty
            FROM `tabBin` bin
            ...
            """,
			(item.item_code, hq_company),
			as_dict=True,
		)
		dealer_qty = dealer_qty_data[0]["dealer_qty"] if dealer_qty_data else 0

		# Get Price List Rate entry for the item. Skip the item if no entry exists.
		...
		if not item_price:
			continue  # Exclude items without a Price List Rate
...
def get_item_group_condition():
	cond = "and 1=1"
	item_groups = ""
	if item_groups:
		placeholders = ", ".join(["%s"] * len(item_groups))
		cond = f"and item.item_group in ({placeholders})"
