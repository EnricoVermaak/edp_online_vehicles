...
function checkClassPresence() {
	...
	const timer = setInterval(function () {
		attempts++;
		const parentDiv = $(
			'.form-group[data-fieldname="show_price_for_request_for_service"]',
		);
		const childrenWithClass = parentDiv.find(".disabled-deselected");
		console.log("Attempt:", attempts);
		console.log("Parent Div:", parentDiv);
		console.log("Children with Class:", childrenWithClass);
...
function toggle_price_visibility(hidePrices) {
	if (hidePrices) {
		$('div[data-fieldname="parts"] .grid-row').each(function () {
			const row = $(this);
			row.find('div[data-fieldname="total_excl"]').hide();
			row.find('div[data-fieldname="price_excl"]').hide();
		});
		$('div[data-fieldname="extras"] .grid-row').each(function () {
			const row = $(this);
			row.find('div[data-fieldname="total_excl"]').hide();
			row.find('div[data-fieldname="price_per_item_excl"]').hide();
		});
		$('div[data-fieldname="labour"] .grid-row').each(function () {
			const row = $(this);
			row.find('div[data-fieldname="total_excl"]').hide();
	...
	setTimeout(() => {
		$('[data-fieldname="show_price_for_request_for_service"]').hide();
	}, 2000);
}
