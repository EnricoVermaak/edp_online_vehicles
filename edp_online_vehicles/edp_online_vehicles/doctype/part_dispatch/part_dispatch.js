...
frappe.ui.form.on("Part Dispatch", {
	refresh: function (frm) {
		...
		// Create or ensure an invisible input element for capturing scan data
		if (
			frm.fields_dict.scanned_part_no &&
			frm.fields_dict.scanned_part_no.$input
		) {
			setTimeout(function () {
				frm.fields_dict.scanned_part_no.$input.focus();
			}, 300);
		}
	...
	part_dispatched: function (frm) {
		...
		if (
			...
			for (let row in frm.doc.parts_dispatched) {
				if (row.part_no == frm.doc.scanned_part_no) {
					pick_flag = false;
					frappe.msgprint("Item already dispatched");
				}
			}

			if (pick_flag) {
				frm.add_child("parts_dispatched", {
				...
				frm.refresh_field("parts_dispatched");
