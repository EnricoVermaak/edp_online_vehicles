...
class VehicleRetail(Document):
    def on_submit(self):
         update_linked_service_plan(self.name)
         update_linked_warranty_plan(self.name)
...
@frappe.whitelist()
def update_linked_service_plan(vehicle_retail_name):
...
@frappe.whitelist()
def update_linked_warranty_plan(vehicle_retail_name):
    doc = frappe.get_doc("Vehicle Retail", vehicle_retail_name)
    activation_date = now_datetime()
...
@frappe.whitelist()
def update_linked_plans_on_submit(vehicle_retail_name):
    update_linked_service_plan(vehicle_retail_name)
    update_linked_warranty_plan(vehicle_retail_name)
    return "Linked Warranty & Service Plans updated successfully"
