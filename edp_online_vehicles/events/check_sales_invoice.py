...
@frappe.whitelist()
def check_sales_invoice_on_payment(doc, event=None):
	settings_doc = frappe.get_doc("Vehicle Stock Settings")

	if settings_doc.allow_stock_allocation_on_invoice_paid == 1:
		for reference in doc.references:
			if reference.reference_doctype == "Sales Invoice":
				if reference.outstanding_amount == 0:
					doc = frappe.get_doc("Sales Invoice", reference.reference_name, ignore_permissions=True)

					for item in doc.items:
						stock_doc = frappe.get_doc(
							"Vehicle Stock", item.custom_vinserial_no, ignore_permissions=True
						)
